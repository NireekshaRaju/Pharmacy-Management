...
from .models import Customer, ContactForm
...
def userdashboard(request,name, cid):
    ...
    return render(request, 'customer/dashboard.html',{'customer':customer,'currentUser':request.session['user_name'],'currentUserId':request.session['customer_id'],'count':request.session['count'],'currentUserEmail':request.session['customer_email'],'customerPhone':request.session['customer_phone']})
...
def index(request,cid):
    ...
    return render(request, 'customer/index.html', {'user_name': user_name,"customer":cust_id,'medicines':medicines,'currentUser':request.session['user_name'],'currentUserId':request.session['customer_id'], 'count':request.session['count'],})
# Signup API View
